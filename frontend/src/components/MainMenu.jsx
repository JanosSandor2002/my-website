...
import { info, websites, university, technologies } from '../data/data';
...
const MainMenu = () => {
  ...
  const getByOffset = (offset) =>
    featuredProjects[(activeIndex + offset + featuredProjects.length) % featuredProjects.length];
    ...
    <section className='landing-page'>
      ...
      <div className='landing-hero'>
        ...
        <p className='landing-subtitle'>
          ...
          Célom letisztult, megbízható és felhasználóbarát megoldásokat készíteni
          valós problémákra — a kódminőség és a vizuális élmény egyaránt fontos.
        </p>
      </div>
      ...
      <div className='landing-grid landing-grid-2'>
        <article className='landing-card landing-card-highlight'>
          ...
          <p>
            Nem csupán kész projektek láthatók itt, hanem az a gondolkodásmód is,
            ahogyan egy ötletből működő termék lesz. A hangsúly a letisztult UI-n,
            a jó felhasználói élményen és az átlátható kódszerkezeten van.
          </p>
        </article>
      ...
      <section className='landing-showcase'>
        <div className='showcase-head'>
          ...
          <p>Nyilakkal lapozhatod a projekteket. A középső kártyán a képre kattintva megnyílik az oldal.</p>
        </div>
        <div className='showcase-carousel'>
          <button className='showcase-nav-btn' onClick={prevProject} aria-label='Előző projekt'>‹</button>
          ...
          <article className='showcase-card showcase-card-active'>
            <a href={center.link} target='_blank' rel='noopener noreferrer' className='showcase-link'>
          ...
          <button className='showcase-nav-btn' onClick={nextProject} aria-label='Következő projekt'>›</button>
        </div>
      </section>
      ...
      <div className='landing-grid landing-grid-2'>
        <article className='landing-card'>
          ...
          <p style={{ marginBottom: '10px', fontSize: '0.83rem', color: 'rgba(232,244,248,0.62)' }}>
            Amelyekkel rendszeresen dolgozom vagy aktívan tanulom.
          </p>
          <div className='tech-dots'>
            {activeTechs.map((t) => (
              <span key={t.id} className='tech-dot'>{t.technology}</span>
            ))}
          </div>
        </article>
        ...
        <article className='landing-card'>
          <h3>Tanulmányi út</h3>
          <p>
            Encsi Vásárhelyi Pál Gimnázium (5 éves nyelvi tagozat) →{' '}
